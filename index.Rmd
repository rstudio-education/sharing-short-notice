...
params:
  github_link: "TBD"
=======
>>>>>>> 13201023220379b3e6edc944c4f9d0622c58e775
---
...
Here we're only using a key for the title, but we could have kept the exact same YAML that we had in any existing single RMD document or we even gotten rid of that and had no YAML at all-- depending on what information we wanted to show. 

The metadata we include in each document is flexible in an RMD site. 
...
Next up, we have `_site.yml`: this is the site's configuration page, and it is basically is a YAML to rule all the other YAMLS.

It is because of *this* file that our index page in our site could be so-pared down.
...
This file is important, so let's pause a little longer here and walk through this:
...
* If you've only ever worked with a single R MArkdown file before, then you've maybe never seen `navbar:` as a YAML key. As you might expect, all of the options indented under here are what build our navbar:
    ...
    + The next key is `left:` (but you can also choose `right:`), and this determines whether your links will go on the left or right side of the navbar.
    
    + Underneath are nested YAML keys where we spell out what items we want in the navbar. 
    
    + Whenever you decide to add a new R Markdown page to your site, you have to remember to add a reference to it here if you want it in your navbar! It's easy to forget to do this.
    
    + As an example: say I have an R Markdown document with Syllabus information that I want to add. The `text:` key is what you'll read in the navbar. And the `href:` (which stands for hypertext reference) is the file location for this page. 
    
**note!** the href should always end in `.html`

    + You can keep growing your navbar by adding pages in this way. 
    
    + And if the navbar starts to get crowded you can use `menu:` key to build a the dropdown menu you saw in our site earlier. This dropdown menu has two items, each links to a lab page. 

+ And finally, there is an output section: 
+ we had this before in the single doc. 
Any keys we apply here will apply to all other parts of our site -- so *this* is why our index.Rmd's YAML was optional. 
...
At a minimum, I recommend having a `title:` and `site: distill::website`, but including a short description or tagline is nice too. 
...
You can have a part of your Distill site be a blog.
...
In other words, you can have a dedicated page on your site that compiles many other R Markdown pages in a chronological way--- this can make it much easier to navigate to a page even if its not in the navbar or a dropdown. 
...
The most recently *dated* R MArkdown doc will pop to the top. This creates a student-centered approach to designing website content -- because students will always be able to see what has been recently updated. This is not always the case with non-blog style pages.
...
You can throw any old existing RMD into the _posts/ folder, change the output to distill_article and be good to go. 

to-do: see if self_contained is necessary. 
