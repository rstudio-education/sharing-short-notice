...
.right-column[
<img src="images/screenshots/build-site.png">
]

???

But before we dig deeper into the files here, let's Build our site and see what we get. 

To do that, you can mouse over to the Build tab, which will build your site in one fell swoop.
...
.right-column[
<img src="images/screenshots/build-site-after.png">
...
Here is the site we just built. We have a navbar and a theme -- and see if you notice anything different about the navbar...we'll come back to that in a bit.

Also homepage link. 

Let's look at the source code for some of the documents that made this site possible, starting with the index.
...
Remember that the index.Rmd is a special R Markdown document. In any site, whatever file you name `index` becomes the homepage.  
...
Here we only have a key for the title, but in fact, if we wanted to, we could have even gotten rid of that and had no YAML at all.  
...
Next up, we have `_site.yml`. And in our balloon analogy, `_site.yml` is the basket that is bringing all the ballon strings together.
...
This file is essentially a mega - YAML, with metadata for the entire website.
...
Here's a Distill site...

Distill is basically an R MArkdown site's sibling but with more technical and scientific interests.

Distill has almost the exact same structure, but it comes with some built-in features that make it easier to author math and science content
...
Screenshot footnotes and asides
...
Screenshot Reuse clause in footer
...
to-do swap build screen shot with Distill proj in IDE, change dir tree
