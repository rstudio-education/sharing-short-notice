...
And it's made up of some metadata, which we call the YAML. The YAML has different keys. One of these Keys is the Title, and it has the value "Lab 02"
...
And then our YAML also has a section all about output. Here's where we specify what format we want (in this case, an HTML document). 
...
But we can also spell out some output options as well -- like if we'd like a table of contents to be generated.  
...
The rest of the file, might have some text in the form of markdown
...
But of course, the real magic happens, when we click on the Knit icon, and see our file turned into HTML. 
